...
test.describe('Export form', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('apps/forms', { waitUntil: 'networkidle' })
		await page.waitForURL(/apps\/forms\/$/)
	})

	test('Export a form as JSON file', async ({ page, appNavigation, form }) => {
		await appNavigation.clickNewForm()
		await form.fillTitle('Export test')
		...
		await appNavigation.getOwnForm('Export test').hover()
		await page.getByRole('button', { name: 'Form actions' }).click()

		// Click Export form in the popover menu
		await page.getByRole('menuitem', { name: 'Export form' }).click()
		...
		expect(json.form.title).toBe('Export test')
	...
	test('Export a form with multiple question types', async ({
		page,
		appNavigation,
		form,
	}) => {
