						...
						<tr
							...
							<td
								...
								<template v-if="questionType === 'number'">
									<NcInputField
										type="number"
										:value="plainValues[row.id][column.id]"
										@input="
											onChangeTextNumber(
												row.id,
												column.id,
...
import { GridAnswerType } from '../../models/Constants.ts'
...
export default {
	...
	data() {
		return {
			...
			isLoading: false,
			questionTypes: [
				{ label: t('forms', 'Radio'), id: GridAnswerType.Radio },
				{ label: t('forms', 'Checkbox'), id: GridAnswerType.Checkbox },
				{ label: t('forms', 'Number'), id: GridAnswerType.Number },
				{ label: t('forms', 'Text'), id: GridAnswerType.Text },
			],
		}
	},
	...
	computed: {
		...
		questionType() {
			return this.extraSettings?.questionType ?? GridAnswerType.Radio
		},
